...
const FEATURE_CARDS = [
  {
    ...
    eyebrowClassName: "text-[#F07C33]",
    iconClassName: "text-[#F07C33]",
  },
  {
    key: "coaching",
        ...
        >
          {visibleCards.map((card) => (
            ...
            >
              ...
              <h3 className={cn("mt-2 text-[#1E1611]", landingTypeTokens.cardTitle)}>{card.title}</h3>
              <p className={cn("mt-5 font-medium text-[#2E231B]", landingTypeTokens.body)}>{card.description}</p>
              ...
              <ul className="mt-6 grid grid-cols-2 gap-2.5">
                {card.chips.map((chip) => (
                  <li
                    key={chip}
                    className={cn(
                      "inline-flex min-h-[42px] items-center gap-1.5 bg-[#F9F5F0] text-[#33281F]",
